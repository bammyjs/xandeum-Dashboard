...
export default function PNodeCard({ node, uptimePct, locationText, variant = 'detailed' }: Props) {
  const statusClass = node.status?.toLowerCase() === 'online' ? 'badge badge-success' : node.status?.toLowerCase() === 'offline' ? 'badge badge-error' : 'badge'
